...
export default class film extends Component {
    ...
    render() {
        return (
            <div className='film'>
                <header>
                    <input type='text' placeholder='请输入影院名称' value={this.state.searchText} onChange={(e)=>{
                        this.setState({
                ...
                <section className='wrapper' style={{
                    ...
                    <ul className='cinemas'>
                        {this.handleFilter().map((item, index) => {
    ...
    handleFilter() {
        ...
        return this.state.cinemasData.filter(item => {
            return item.name.toUpperCase().includes(this.state.searchText.toUpperCase()) || item.address.toUpperCase().includes(this.state.searchText.toUpperCase())
        }, )
    }
